## This script will collect the base layer for the NDVI project. 
## The base layer will be a mosaic of Red, Green, and Blue bands from Sentinel-2 imagery. 
## The script will search for scenes with the least cloud cover in Missoula County, Montana, and create a mosaic of the Red, Green, and Blue bands. 
## The final RGB mosaic will be saved as a GeoTIFF file.
...
mso_county_geojson_UTM = mapping(mso_county_gdf.geometry[0])  # Convert to GeoJSON format
...
for scene in items:
    ...
    assets = scene.assets
    process_band(assets['red'].href, 'red', scene.id, mso_county_geojson_UTM)
    process_band(assets['green'].href, 'green', scene.id, mso_county_geojson_UTM)
    process_band(assets['blue'].href, 'blue', scene.id, mso_county_geojson_UTM)
